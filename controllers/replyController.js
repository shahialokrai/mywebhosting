...
//show a reply by a forumid
const showByForum = (req, res, next) => {
  let forumId = req.body.forumId;

  Reply.find({ forumid: forumId })
    .populate("userid", "-password")
    .then((response) => {
      res.json({
        response,
...
const store = (req, res, next) => {
  ...
  reply
    ...
    .then((response) => {
      res.json({
        message:
          "forumreply " + req.body["forumreply"] + " is Added Successfully!",
      });
...
const update = (req, res, next) => {
  ...
  Reply.findByIdAndUpdate(replyId, { $set: updatedData })
    .then(() => {
      res.json({
        message:
          "forumreply  " + req.body["forumreply"] + " is updated Successfully!",
      });
      console.log(req.body["forumreply"]);
    })
    .catch((error) => {
